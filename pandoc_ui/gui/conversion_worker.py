        ...
        try:
            ...
            if not self.service.is_pandoc_available():
                result = ConversionResult(
                    success=False,
                    error_message="Pandoc is not available on this system"
                )
                self.conversion_finished.emit(result)
                return
            
            pandoc_info = self.service.get_pandoc_info()
            self.log_message.emit(f"✅ Pandoc detected: {pandoc_info.path} (v{pandoc_info.version})")
            ...
            if not self.service.validate_input_file(self.profile.input_path):
                result = ConversionResult(
                    success=False,
                    error_message=f"Invalid input file: {self.profile.input_path}"
                )
                self.conversion_finished.emit(result)
                return
