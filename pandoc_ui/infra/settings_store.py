...
class ApplicationSettings(BaseModel):
    ...
    # Default Paths
    default_input_dir: Optional[str] = Field(default=None, description="Default input directory")
    default_output_dir: Optional[str] = Field(default=None, description="Default output directory")
    recent_input_files: List[str] = Field(default_factory=list, max_items=10)
    recent_output_dirs: List[str] = Field(default_factory=list, max_items=10)
    
    # Processing Settings
    max_concurrent_jobs: int = Field(default=4, ge=1, le=16, description="Max concurrent conversion jobs")
    default_output_format: str = Field(default="html", description="Default output format")
    default_extensions: str = Field(default=".md,.markdown,.txt", description="Default file extensions for batch")
    default_recursive_scan: bool = Field(default=True, description="Default recursive scanning mode")
    max_batch_files: int = Field(default=1000, ge=1, le=10000, description="Maximum files in batch")
    
    # Advanced Settings
    pandoc_timeout_seconds: int = Field(default=300, ge=10, le=3600, description="Pandoc execution timeout")
    log_level: str = Field(default="INFO", description="Logging level")
    auto_save_profiles: bool = Field(default=True, description="Auto-save profiles on changes")
    confirm_batch_operations: bool = Field(default=True, description="Confirm large batch operations")
    
    # Performance Settings
    enable_progress_callbacks: bool = Field(default=True, description="Enable progress reporting")
    ui_update_interval_ms: int = Field(default=100, ge=50, le=1000, description="UI update interval")
    
    @validator('recent_input_files', 'recent_output_dirs')
    def validate_recent_paths(cls, v):
...
class SettingsStore:
    ...
    def save_settings(self, settings: Optional[ApplicationSettings] = None) -> bool:
        """
