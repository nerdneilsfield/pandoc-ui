...
class TestGUIIntegration:
    ...
    def test_ui_components_initialized(self, main_window):
        ...
        assert hasattr(ui, "inputPathEdit")
    ...
    def test_format_combo_initialization(self, main_window):
        ...
        # Check that essential formats are available
        format_texts = [combo.itemText(i) for i in range(combo.count())]
        essential_formats = ["HTML", "PDF", "Microsoft Word", "OpenOffice/LibreOffice", "EPUB", "LaTeX", "Rich Text Format"]
        
        for expected_format in essential_formats:
            assert any(expected_format in format_text for format_text in format_texts), f"Format '{expected_format}' not found"
    ...
    def test_file_selection_workflow(self, main_window):
        ...
        handler.ui.inputPathEdit.setText(str(test_file))
    ...
    def test_status_updates(self, main_window):
        ...
        test_statuses = ["Ready", "Converting...", "Conversion completed", "Error occurred"]
    ...
    def test_conversion_without_file(self, mock_msgbox, main_window):
        ...
        assert "Please select a valid input file" in args[2]
    ...
    def test_conversion_button_state_changes(self, main_window):
        ...
        handler.ui.inputPathEdit.setText("test.md")
        handler.updateConvertButtonState()
        assert handler.ui.convertButton.isEnabled()
    ...
    def test_pandoc_availability_integration(self, main_window):
        ...
        # Check that UI can be updated correctly
        status_text = handler.ui.statusLabel.text()
        assert "Ready - Pandoc available" == status_text
