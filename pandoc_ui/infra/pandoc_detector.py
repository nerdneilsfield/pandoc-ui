...
class PandocDetector:
    ...
    def __init__(self) -> None:
        self._cached_info: PandocInfo | None = None
    ...
    def detect(self) -> PandocInfo:
        ...
        logger.debug(
            f"Searching {len(search_paths)} locations: {[str(p) for p in search_paths[:5]]}..."
        )
        ...
        if system.lower() == "windows":
            ...
            if path_found:
                ...
                if version:
                    logger.info(
                        f"Manual PATH search found working pandoc: {path_found} (version: {version})"
                    )
    ...
    def _get_version(self, pandoc_path: Path) -> str | None:
        """
    ...
    def _manual_path_search_windows(self) -> Path | None:
        """
                        ...
                        if pandoc_path.exists() and pandoc_path.is_file():
                            ...
                            try:
                                ...
                                if result.returncode == 0 and "pandoc" in result.stdout.lower():
                                    logger.info(
                                        f"Manual search verified working pandoc: {pandoc_path}"
                                    )
                                    return pandoc_path
                                else:
                                    logger.debug(
                                        f"File exists but not working pandoc: {pandoc_path}"
                                    )
                            except Exception as e:
