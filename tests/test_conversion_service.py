...
class TestConversionService:
    ...
    def test_convert_success(self, mock_get_runner):
        ...
        mock_result = ConversionResult(
            success=True, output_path=Path("output.html"), duration_seconds=1.5
        )
        mock_runner.execute.return_value = mock_result
        mock_get_runner.return_value = mock_runner

        profile = ConversionProfile(input_path=Path("input.md"), output_format=OutputFormat.HTML)
    ...
    def test_convert_failure(self, mock_get_runner):
        ...
        mock_runner = Mock()
        mock_result = ConversionResult(success=False, error_message="Conversion failed")
        mock_runner.execute.return_value = mock_result
        mock_get_runner.return_value = mock_runner
